...
public class UserServiceImpl implements UserService{
    ...
    public Messenger login(UserDTO userDTO) {
        ...
        .peek(i -> i.setToken(tokenRepository.save(
                                ...
                                .refreshToken(jwtProvider.createRefreshToken(UserDTO.builder().id(i.getId()).name(i.getName()).role(i.getRole()).build()))
                                .build())))
        .map(i -> userRepository.save(i))
        .map(i -> Messenger.builder()
            ...
            .accessToken(jwtProvider.createAccessToken(UserDTO.builder().id(i.getId()).role(i.getRole()).build()))
            .build())
        .findAny()
