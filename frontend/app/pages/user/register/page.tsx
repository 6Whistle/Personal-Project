...
import { checkEmailAPI, findUserByIdAPI, loginUserAPI } from "@/app/component/user/service/user-api";
import { registerUser } from "@/app/component/user/service/user-service";
import { getUserJson, getUserStatus } from "@/app/component/user/service/user-slice";
...
export default function RegisterUser(){
  ...
  const onSubmit : SubmitHandler<User> = (data) => 
    ...
    .then(({payload}: any) => {
      ...
      payload.status === 200 ? router.push("/") : console.log("Register failed : ", payload.status)
    })
              ...
              <div className=" flex flex-col items-center">
                ...
                className="w-full flex-1 mt-8">
                  <div className="mx-auto max-w-xs flex flex-col gap-4">
                    { registerList.map((item, index) => <RegisterInput inputType={item.inputType} placeholder={item.placeholder} target={item.target} register={register} key={index}/>) }
                    <RegisterButton />
